...
import * as styles from "./AdminMyCatalogiDetailTemplate.module.css";
import {
  Button,
  Heading1,
  Heading2,
...
export const AdminMyCatalogiDetailTemplate: React.FC<AdminMyCatalogiDetailTemplateProps> = ({ myCatalogiId }) => {
  const { t } = useTranslation();
  const [currentTab, setCurrentTab] = React.useState<number>(0);
  const [filters, setFilters] = React.useContext(FiltersContext);

  const tempComponent =
    TEMPORARY_COMPONENTS.find((component: any) => component.id === myCatalogiId) ?? TEMPORARY_COMPONENTS[1];
