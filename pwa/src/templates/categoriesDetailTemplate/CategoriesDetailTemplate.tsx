...
export const CategoriesDetailTemplate: React.FC<CategoriesDetailTemplateProps> = ({ categoryId }) => {
  ...
  const [viewType, setViewType] = React.useState<"cards" | "layers">("cards");
  ...
  const components: any[] = (portfolio && TEMPORARY_COMPONENTS) || [];

  const badgeNumber = components.length;
    ...
    <Container layoutClassName={styles.container}>
      ...
      {components && (
        <div className={styles.solutions}>
          ...
          {viewType === "layers" && <ComponentCardsAccordionTemplate components={components} />}
        </div>
      )}
